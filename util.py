...
print("filed offset:", FIELD_OFFSETS)
...
def gather_2d(params, indices): # params: M*N  indices: (M*k) * 2
    shape = tf.shape(params) # shape = [M, N]
    flat = tf.reshape(params, [-1]) # flat=[d00, d01... d0N-1, ...dM-10, ... dM-1N-1]
    flat_idx = indices[:, 0] * shape[1] + indices[:, 1]
    flat_idx = tf.reshape(flat_idx, [-1]) # flat_idx=[idx_row0_tok1, ... idx_row0_topk, ... idsx_rowm-1topk]
...
# 池化2d
def max_pool_2d(params, k): # params: M*N
    _, indices = tf.nn.top_k(params, k, sorted=False) # indices: M*k [[r11,...r1k],...[rm1,...rmk]]
    shape = tf.shape(indices) # shape = (M, k)
    r1 = tf.reshape(tf.range(shape[0]), [-1, 1]) # r1: M*1 [[0], [1], [M-1]]
    r1 = tf.tile(r1, [1, k]) # r1: M*k [[0,0,0], ... [M-1,M-1,M-1]]
    r1 = tf.reshape(r1, [-1, 1]) # r1: (M*k) * 1 [[0],[0],[0]...[M-1],[M-1],[M-1]]
    indices = tf.concat([r1, tf.reshape(indices, [-1, 1])], 1) 
    # indices: (M*k) * 2 [[0,r11],...[0, r1k],...[m-1, rm1], ... [m-1, rmk]]
